...
def plot_convergence_comparison(convergences, redshifts):
    ...
    for ax, conv, z in zip(axes, convergences, redshifts):
        ...
        plt.colorbar(im, ax=ax, label='Îº', fraction=0.046)
...
def main():
    ...
    # Modules are required; if import failed, this page would not load
    ...
    **Multi-plane gravitational lensing** accounts for the realistic 3D distribution of matter
    ...
    # Configuration
    st.subheader("™ï¸ Multi-Plane Configuration")
    ...
    st.markdown("---")
    st.subheader("ðŸ”­ Lens Plane Parameters")
    ...
    if 'multi_source_pos' in st.session_state:
        ...
        col1.metric("Magnification", f"{magnification:.2f}Ã—")
        col2.metric("Source RMS", f"{np.std(source_pos):.3f}\"")
        col3.metric("Image RMS", f"{np.std(image_pos):.3f}\"")
        
        # Convergence maps
        with st.expander("ðŸ—ºï¸ View Convergence Maps per Plane"):
